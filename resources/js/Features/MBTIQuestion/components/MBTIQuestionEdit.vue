<template>
    <div
        ...
        <label for="question_text" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">質問テキスト</label>
        <input
            type="text" v-model="question_text"
            id="question_text"
...
const schema = object({
    question_text: string().required('質問テキストは必須項目です'),
...
const {handleSubmit, errors} = useForm({
    ...
    initialValues: {
        question_text: props.mbti_question.question_text,
        answer_a: props.mbti_question.answer_a,
        answer_b: props.mbti_question.answer_b
    }
});

const {value: question_text, errorMessage: questionTextError} = useField('question_text');
